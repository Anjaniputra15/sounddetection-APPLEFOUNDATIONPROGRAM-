...
class ClassificationResultsSubject: NSObject, SNResultsObserving {
    private let subject: PassthroughSubject<SNClassificationResult, Error>
     var confidenceResultSubject:Double
    init(subject: PassthroughSubject<SNClassificationResult, Error>) {
        self.subject = subject
        self.confidenceResultSubject = 0.0
    }
    ...
    func request(_ request: SNRequest,
        ...
        if let result = result as? SNClassificationResult {
            subject.send(result)
            let sNResult = result.classification(forIdentifier: "bird_vocalization")!
            confidenceResultSubject = sNResult.confidence
//            UserDefaults.standard.set(confidenceResultSubject, forKey: "confidenceResultSubject")
            BridDetectionSUITileApp.currentBirdSoundConfidence = confidenceResultSubject
        }
    }
}
